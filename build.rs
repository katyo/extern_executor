fn main() {
    ...
    #[cfg(feature = "cbindgen")]
    {
        ...
        std::fs::create_dir_all(&header_dir).expect("Unable to create headers directory");

        for (needed, header_file, sym_filter) in &[
            (
                true,
                "rust_async_executor.h",
                (|name| !name.contains("_dart_") && !name.contains("_uv_")) as fn(&str) -> bool,
            ),
            (cfg!(feature = "uv"), "rust_async_executor_uv.h", |name| {
                name.contains("_uv_")
            }),
            (
                cfg!(feature = "dart"),
                "rust_async_executor_dart.h",
                |name| name.contains("_dart_"),
            ),
        ] {
            if *needed {
